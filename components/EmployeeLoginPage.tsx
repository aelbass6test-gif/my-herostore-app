      ...
      <div className="w-full max-w-md bg-slate-900/40 p-8 rounded-2xl border border-slate-700 shadow-xl backdrop-blur-sm transition-all">
        {view === 'login' && (
          <div className="animate-in fade-in duration-300">
            ...
            <p className="text-center text-sm text-slate-400 mt-6 pt-4 border-t border-slate-800">
                <button onClick={() => setView('register')} className="font-bold text-indigo-400 hover:underline">موظف جديد؟ سجل طلب انضمام لمتجر</button>
                <br/>
                <Link to="/owner-login" className="text-xs text-slate-500 hover:underline mt-2 inline-block">هل أنت مالك المتجر؟</Link>
            </p>
          </div>
        )}
        ...
        {view === 'register' && (
          <div className="animate-in fade-in duration-300">
            ...
            <p className="text-center text-sm text-slate-400 mt-6 pt-4 border-t border-slate-800">
                لديك حساب بالفعل؟ <button onClick={() => setView('login')} className="font-bold text-indigo-400 hover:underline">تسجيل الدخول</button>
            </p>
          </div>
        )}
        ...
        {view === 'success' && (
            <div className="animate-in fade-in duration-300 text-center">
                ...
                <button onClick={() => setView('login')} className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:opacity-90 text-white rounded-lg py-3 font-bold transition-colors">
                    العودة لصفحة الدخول
                </button>
            </div>
        )}
